...
class Air_forecast_performance extends MY_Controller {
    ...
    function index() {
        ...
        $this->Logs_model->add_log($this->login_user->client_id, NULL, NULL, NULL, 'Access_forecast_Performance');
		...
		// GRÁFICOS ESTACIONES
		$stations = array();
		foreach(CONST_ARRAY_NO_EYE3_STATIONS_IDS as $id_station) {
			
			$station = $this->Air_stations_model->get_one($id_station);
			$stations[] = $station;
			$view_data["code_api_sgs"][$station->id] = $station->code_api_sgs;

			// LLAMAR A LA CONFIGURACIÓN DE ALERTAS DE PRONÓSTICO DE LA ESTACIÓN ITERADA Y LA VARIABLE PM10
			...
			$chart_neur_data_p = $this->_get_data_chart_forecast($options);
			$view_data["chart_neur_model_values_p"][$station->id] = $chart_neur_data_p["chart_model_values_p"];
			$view_data["chart_neur_model_ranges_p"][$station->id] = $chart_neur_data_p["chart_model_ranges_p"];
			$view_data["chart_neur_model_intervalo_confianza"][$station->id] = $chart_neur_data_p["chart_model_intervalo_confianza"];
			$view_data["chart_neur_model_porc_conf"][$station->id] = $chart_neur_data_p["chart_model_porc_conf"];
			...
			$chart_ml_data_p = $this->_get_data_chart_forecast($options);
			$view_data["chart_ml_model_values_p"][$station->id] = $chart_ml_data_p["chart_model_values_p"];
			$view_data["chart_ml_model_ranges_p"][$station->id] = $chart_ml_data_p["chart_model_ranges_p"];
			$view_data["chart_ml_model_intervalo_confianza"][$station->id] = $chart_ml_data_p["chart_model_intervalo_confianza"];
			$view_data["chart_ml_model_porc_conf"][$station->id] = $chart_ml_data_p["chart_model_porc_conf"];
			$view_data["chart_ml_model_formatted_dates"][$station->id] = $chart_ml_data_p["chart_model_formatted_dates"];
			/* MODELO MACHINE LEARNING */
			...
			$chart_num_data_p = $this->_get_data_chart_forecast($options);
			$view_data["chart_num_model_values_p"][$station->id] = $chart_num_data_p["chart_model_values_p"];
			$view_data["chart_num_model_ranges_p"][$station->id] = $chart_num_data_p["chart_model_ranges_p"];
			$view_data["chart_num_model_intervalo_confianza"][$station->id] = $chart_num_data_p["chart_model_intervalo_confianza"];
			$view_data["chart_num_model_porc_conf"][$station->id] = $chart_num_data_p["chart_model_porc_conf"];
		...
		usort($stations, function($objeto1, $objeto2){
			$posicion1 = array_search($objeto1->id, array(2, 1, 3, 4, 13));
			$posicion2 = array_search($objeto2->id, array(2, 1, 3, 4, 13));
			return $posicion1 - $posicion2;
		});

		$view_data["stations"] = $stations;

        $this->template->rander("air_forecast_performance/index", $view_data);
	}
	...
	function get_sgs_monitoring_data(){
		...
		foreach($period as $datetime){
			...
			$time = $datetime->format("H");
			$chart_data_m[$date." ".$time.":00 hrs"] = 0;
		}
        ...
        if ($client->fault) {
			echo json_encode(array("success" => false, 'message' => "Error en la solicitud a API. Código de Error: ".$result["faultcode"].". "."Mensaje de Error: ".$result["faultstring"]));
			exit();
            ...
            if($error){ // SI HAY UN ERROR EN LA RESPUESTA, LO MUESTRA
                ...
                foreach($result_data as $key => $rd){
					...
					$srv_cod = trim(utf8_encode($rd["SrvCod"]));

                    if($sigla_variable != "MP10 Esampler" && $sigla_variable != "MP10"){ continue; } // SOLO MOSTRAR DATOS DE VARIABLE MP10

					// SI LA ESTACIÓN ES CUNCUMÉN, CONSIDERAR EL VALOR DE MP10 PARA EL OBJETO QUE TENGA EL VALOR "10003439-9" EN LA LLAVE SvrCod 
					if($api_station_code == "2009" && $srv_cod != "10003439-9"){ continue; }
	...
	function get_meteodata_monitoring_data(){

		$api_station_code = $this->input->post("api_station_code"); // "mlp_es_hm" (Hotel Mina)

		$first_datetime = new DateTime('now', new DateTimeZone('UTC'));
		$first_datetime->setTimezone(new DateTimeZone('America/Santiago'));
		$first_datetime = $first_datetime->modify('-48 hours');
		$first_date = $first_datetime->format("d-m-Y");
		$first_date_api = $first_datetime->format("Ymd_000000");

		$last_datetime = new DateTime('now', new DateTimeZone('UTC'));
		$last_datetime->setTimezone(new DateTimeZone('America/Santiago'));
		$last_date = $last_datetime->format("d-m-Y");
		$last_date_api = $last_datetime->format("Ymd_235000");
		...
		$params = array(
			"op" => "datos",
			"est" => $api_station_code,
			"var" => "mp10",
			"tini" => $first_date_api,
			'tfin' => $last_date_api
		);

		$url = "https://pelambres.meteodata.cl/met/consultas?".http_build_query($params);
        $token = "Token 4dc5af4bdc28cddb3d0f28379d4c9bdf7d4edcf9";
		...
		$array_response = json_decode($response)->datos;
		curl_close($ch);
		...
		if ($response === false) {
			...
			$error = curl_error($ch);
			echo json_encode(array("success" => false, 'message' => "Error en la solicitud a API: ".$error));
			exit();

		} else {

			$array_data = array();
			foreach($array_response as $data){
				$datetime = $data[0];
				$value = $data[1];
				$array_data[$datetime] = $value;
			}

			$averaged_hours = array();

			foreach($array_data as $datetime => $value) {

				$datetime_obj = new DateTime($datetime);
				$hour = $datetime_obj->format('d/m/Y H:00');
				$hour = $hour. " hrs";
				if (!isset($averaged_hours[$hour])) {
					$averaged_hours[$hour] = array('values' => array($value), 'avg' => 0);
				} else {
					$averaged_hours[$hour]['values'][] = $value;
				}
			}

			foreach($averaged_hours as $hour => &$data) {
				$avg = count($data['values']) > 0 ? array_sum($data['values']) / count($data['values']) : 0;
				$data['avg'] = $avg;
			}

			// Imprimir el nuevo arreglo con los avgs por hora

			foreach ($averaged_hours as $hour => $data) {
				// echo "Hora: $hour, avg: " . $data['avg'] . "<br>";
				$chart_data_m[$hour] = $data['avg'];
			}
