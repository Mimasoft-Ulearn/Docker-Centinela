...
class Air_forecast_comparison extends MY_Controller {
    ...
    function index() {
        ...
        $this->Logs_model->add_log($this->login_user->client_id, NULL, NULL, NULL, 'Access_forecast_Comparison');
        ...
        foreach ($air_stations as $station) {
			...
			if (in_array($station->id, CONST_ARRAY_NO_EYE3_STATIONS_IDS)) {
				...
				$stations_api_code[$station->id] = $station->code_api_sgs;
			}
		}

		// ORDENAR EL ARREGLO DE OBJETOS DE ESTACIONES POR ID EN EL ORDEN SOLICITADO: array(2, 1, 3, 4)
		...
		foreach($stations as $station){
			...
			if($station->id == 1){ // COM
				$array_alerts_yAxis_tickPositions[] = 1500;
				$array_alerts_yAxis_tickPositions[] = 1501;
			}
	...
	function get_sgs_monitoring_data(){
		...
		// echo json_encode(array("success" => false, 'message' => "Solicitud cancelada para realizar pruebas (borrar)"));
		// exit();
