					...
					if($id_model == 3 && $file_basename == "num_mapa"){ // Numérico && Ninguno (Mapa)
						...
						if($variable->id_air_variable_type == 1){ // Variable Meteorológica
							...
							for($column = 3; $column <= $ultima_columna_lat_n; $column++){
								...
								$valor_columna = $worksheet->getCell($letra_columna.'3')->getValue();
							...
							// CELDA FECHA MODELO
							$fecha_excel = $worksheet->getCell('A1')->getValue();
							//Y-m-d H:i
							if($this->validateDate($fecha_excel, 'Y-m-d H:i')){
							...
							for($column = 2; $column <= $ultima_columna_lat_n; $column++){
								...
								// FILA LONGITUDES
								$letra_columna = PHPExcel_Cell::stringFromColumnIndex($column - 1);
								$valor_columna = $worksheet->getCell($letra_columna.'4')->getValue();
						...
						if($variable->id_air_variable_type == 1){ // Variable Meteorológica
							...
							for($row = 4; $row <= $lastRow; $row++){
								...
								for($column = 3; $column <= $ultima_columna_lat_n; $column++){
									...
									// LONGITUD
									$longitud = $worksheet->getCell($letra_columna.'3')->getValue();
							...
							for($row = 5; $row <= $lastRow; $row++){
								...
								for($column = 2; $column <= ($ultima_columna_lat_n); $column++){
									...
									$valor_columna_lat = $worksheet->getCell($letra_columna.'3')->getValue();
									$valor_columna_long = $worksheet->getCell($letra_columna.'4')->getValue();
