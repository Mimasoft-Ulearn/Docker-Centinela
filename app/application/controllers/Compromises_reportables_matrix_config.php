...
class Compromises_reportables_matrix_config extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("compromises_reportables_matrix_config/index", $view_data);
    }
	...
	function modal_form() {
		...
		$view_data['campos_compromiso'] = array(); 
		
		if($id_compromiso){ //edit
			$view_data['campos_compromiso'] = $this->Compromises_reportables_rel_fields_model->get_compromise_fields($id_compromiso)->result_array();
			$view_data['evaluados_compromiso'] = $this->Plans_reportables_compromises_model->get_all_where(array("id_compromiso" => $id_compromiso, "deleted" => 0))->result_array();
		}
		...
		$valores_compromiso = $this->Values_compromises_reportables_model->get_all_where(array("id_compromiso" => $id_compromiso, "deleted" => 0))->result();
        ...
        $this->load->view('compromises_reportables_matrix_config/modal_form', $view_data);
	...
	function save() {
		...
		$id_compromiso = $this->Compromises_reportables_model->get_one_where(array("id_proyecto" => $id_proyecto, "deleted" => 0))->id;
		...
		if (!$id_compromiso) { //insert
			...
			$save_id = $this->Compromises_reportables_model->save($data_compromiso);
			...
			foreach($campos as $campo){
				...
				$save_id_campo = $this->Compromises_reportables_rel_fields_model->save($data_campos);
			}
			...
			//} else {
				...
				$save_id = $this->Compromises_reportables_model->save($data_compromiso, $id_compromiso);
			
				$delete_campos_compromiso = $this->Compromises_reportables_rel_fields_model->delete_fields_related_to_compromise($id_compromiso);
		...
		$data_project = array("matriz_compromisos_reportables" => 1);
	...
	function list_data() {
		...
		$options = array(
			...
			"matriz_creada_reportables" => $this->input->post("matriz_creada_reportables")
		);
		
		$list_data = $this->Projects_model->get_available_compromises_projects($id_compromises_module, $options, "reportables")->result();
	...
	private function _row_data($id_proyecto) {
		...
		$data = $this->Projects_model->get_available_compromises_projects($id_compromises_module, $options, "reportables")->row();
	...
	private function _make_row($data) {
		...
		$id_compromiso = $this->Compromises_reportables_model->get_one_where(array("id_proyecto" => $data->id, "deleted" => 0))->id;
		
		//SI EXISTEN EVALUACIONES DE COMPROMISOS DE LA MATRIZ, BLOQUEAR EL BOTÓN DE EDICIÓN.
		$hay_evaluaciones = FALSE;
		
		// SI NO EXISTE AL MENOS UN ESTADO CON CATEGORIA NO CUMPLE, BLOQUEAR EL BOTÓN DE INGRESO
		$project_info = $this->Projects_model->get_one($data->id);
		$no_cumple = $this->Compromises_compliance_status_model->get_all_where(
			array(
		...
		$hay_estado_no_cumple = (bool)$no_cumple;

		// SI NO EXISTE AL MENOS UN ESTADO CON CATEGORIA PENDIENTE, BLOQUEAR EL BOTÓN DE INGRESO
		$project_info = $this->Projects_model->get_one($data->id);
		$pendiente = $this->Compromises_compliance_status_model->get_all_where(
			array(
				"id_cliente" => $project_info->client_id, 
				"categoria" => "Pendiente", 
				"tipo_evaluacion" => "reportable", 
		...
		$hay_estado_pendiente = (bool)$pendiente;
		
		$valores_compromiso = $this->Values_compromises_reportables_model->get_all_where(array(
		...
		if($data->matriz_compromisos_reportables == 1){ //proyecto que tiene habilitado el módulo Compromisos
			
			$boton_ver = modal_anchor(get_uri("compromises_reportables_matrix_config/view/" . $data->id), "<i class='fa fa-eye'></i>", array("class" => "edit", "title" => lang('view_matrix'), "data-post-id" => $data->id,  "data-post-id_compromiso" => $id_compromiso));
			if($hay_evaluaciones || (!$hay_estado_no_cumple || !$hay_estado_pendiente)){
				$boton_editar = '<span style="cursor: not-allowed;">' . modal_anchor(get_uri(/* "compromises_reportables_matrix_config/modal_form" */), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_matrix'), "data-post-id" => $data->id, "style" => "pointer-events: none; background-color: #dcdcdc;")).'</span>';
			} else {
				$boton_editar = modal_anchor(get_uri("compromises_reportables_matrix_config/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_matrix'), "data-post-id" => $data->id, "data-post-id_compromiso" => $id_compromiso));
			}
			$boton_agregar = '<span style="cursor: not-allowed;">'. modal_anchor(get_uri(/* "compromises_reportables_matrix_config/modal_form" */), "<i class='fa fa-plus'></i>", array("class" => "edit", "title" => lang('add_matrix'), "data-post-id" => $data->id, "style" => "pointer-events: none; background-color: #dcdcdc;")).'</span>';
			$matriz_creada = '<i class="fa fa-check" aria-hidden="true"></i>';
			
		} else {
			
			$boton_ver = '<span style="cursor: not-allowed;">' . modal_anchor(get_uri(/* "compromises_reportables_matrix_config/view/" . $data->id */), "<i class='fa fa-eye'></i>", array("class" => "edit", "title" => lang('view_matrix'), "data-post-id" => $data->id, "style" => "pointer-events: none; background-color: #dcdcdc;")).'</span>';
			
			$boton_editar = '<span style="cursor: not-allowed;">' . modal_anchor(get_uri(/* "compromises_reportables_matrix_config/modal_form" */), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_matrix'), "data-post-id" => $data->id, "style" => "pointer-events: none; background-color: #dcdcdc;")).'</span>';
			
			if(!$hay_estado_no_cumple || !$hay_estado_pendiente){
				$boton_agregar = '<span style="cursor: not-allowed;">' . modal_anchor(get_uri("compromises_reportables_matrix_config/modal_form"), "<i class='fa fa-plus'></i>", array("class" => "edit", "title" => lang('add_matrix'), "data-post-id" => $data->id, "style" => "pointer-events: none; background-color: #dcdcdc;")).'</span>';
			}else{
				$boton_agregar = modal_anchor(get_uri("compromises_reportables_matrix_config/modal_form"), "<i class='fa fa-plus'></i>", array("class" => "edit", "title" => lang('add_matrix'), "data-post-id" => $data->id));
			}
        ...
        $info = (!$hay_estado_no_cumple || !$hay_estado_pendiente) ? '<span class="help" data-container="body" data-toggle="tooltip" title="'.lang('disabled_matrix_actions_rep_msj').'"><i class="fa fa-info-circle"></i></span>' : '';
	...
	function view($id_proyecto = 0) {
        ...
        if ($id_compromiso_proyecto) {
			...
			$compromiso = $this->Compromises_reportables_model->get_one($id_compromiso_proyecto);
			...
			$this->load->view('compromises_reportables_matrix_config/view', $view_data);
