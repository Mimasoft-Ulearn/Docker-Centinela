...
class Cron_job {
	...
	// Alertas MIMAire
	function run_air_alerts($id_proyecto, $force_send = false, $get_email_content = false){
		...
		// Si en la fecha actual no se ha enviado los correos de alerta, ó, se fuerza el envío
		if(!$alert_historical_air->id || $force_send){
			...
			if(count($array_forecast_email_data)){
				...
				if($get_email_content){
					return $this->_get_email_forecast_alert_html_content($id_proyecto, 1, $array_forecast_email_data);
					exit();
				} else {
					...
					$send_email_air_forecast_alert = $this->_send_email_air_forecast_alert_PHPMailer($id_proyecto, 1, $array_forecast_email_data);
				}
			...
			return $send_email_air_forecast_alert;
	...
	*/
	...
	/* Version con PHPMailer
		Version de la función que envía en cada correo una lista con los demás usuarios que recibieron el correo.
		NOTA: esta función asume que todos los usuarios notificados tienen las mismas alertas asociadas.
	*/
	function _send_email_air_forecast_alert_PHPMailer($id_proyecto, $id_modelo, $array_forecast_email_data){
		...
		if($array_forecast_email_data["no_data"] == true){
			...
			foreach($array_forecast_email_data as $index => $id_user){
				if($index != "no_data"){
					...
					$message = $this->ci->parser->parse_string($email_template->message, $parser_data, TRUE);
					$send_app_mail = send_app_mail_PHPMailer($user_to_notify->email, $email_template->subject, $message, $email_options);
			...
			foreach($array_forecast_email_data_by_user as $id_user => $array_forecast_email_data){
				...
				$message = $this->ci->parser->parse_string($email_template->message, $parser_data, TRUE);
				$send_app_mail[$id_user] = send_app_mail_PHPMailer($user_to_notify->email, $email_template->subject, $message, $email_options);
	...
	function _get_email_forecast_alert_html_content($id_proyecto, $id_modelo, $array_forecast_email_data){
	...
	function enviar_correo_verificacion(){
		...
		$recipients = array(
			"natalia.ulearn@outlook.com",
			"gustavo@mimasoft.cl",
			"christopher@mimasoft.cl",
			"cst.samv@gmail.com",
			"carla.bravo@ulearn.cl",
			"roberto.alveal@ulearn.cl",
		);
		
		$subject = "Mensaje de prueba de servidor de correos";
		$message = "Este mensaje es para verificar el correcto funcionamiento del servidor de correos. Se ha programado para ejecutarse todos los días a las 9 am. No debes contestar a este correo. ¡Que tengas un buen día!.";
		$filepath = getcwd()."/files/system/dummy.pdf";
		$options = array("attachments" => array(array("file_path" => $filepath, "file_name" => 'pdf_prueba.pdf')));

		foreach ($recipients as $to) {
			
			send_app_mail_PHPMailer($to, $subject, $message, $options);
		}
	}
