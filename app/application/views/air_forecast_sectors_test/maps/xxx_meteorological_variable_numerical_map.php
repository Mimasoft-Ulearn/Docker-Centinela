...
<script type="text/javascript">
	...
	// CREO LA INSTANCIA DE MAPA
	var map = new L.Map('mapa', {
		center: new L.LatLng(-32.86715, -71.02835),
		zoom: 13,
			...
			if(fecha == "<?php echo $date; ?>"){
				...
				<?php foreach($values_p as $hora => $valor){ ?>
					...
					if(hora_f == "<?php echo $hora; ?>"){
						...
						console.log('<?php echo $lat.":".$lon." | ".$valor; ?>');
		...
		heatmapLayer.setData({ // Se setea el layer Heatmap con los datos de un tiempo determinado (time)
			...
			data: [
				...
				<?php foreach($map_data as $data){ ?>
					console.log("<?php echo $data["lat"].":".$data["lon"]." | ".$data["valor"]?>");

					{lat: <?php echo $data["lat"]; ?>, lon: <?php echo $data["lon"]; ?>, cont: <?php echo $data["valor"] ? $data["valor"] : 0; ?>},
				<?php } ?>
