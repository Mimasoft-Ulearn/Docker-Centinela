        ...
        <table id="tabla" class="table table-bordered table-hover">
            <thead>
                ...
                <tr>
                    ...
                    <th class="th_border text-center"><?php echo lang("none"); ?><br /><input type="radio" id="radio__airselect_delete_none" name="radio_air_select_delete" /></th>
                </tr>
            </thead>
            <tbody>
            <?php foreach ($air_modules as $key => $module) { ?>
                ...
                <?php
					...
					//Deshabilitar radio a módulos que no tienen tableros para añadir, editar o borrar.
					if($module->id_client_submodule == 23 || $module->id_client_submodule == 25 || $module->id_client_submodule == 26){
						$disabled = "disabled";
					} else {
						$disabled = "";
					}
					...
					// Desactiva: Permiso VER: propios, Permiso AÑADIR: completo, Permiso EDITAR: propios y BORRAR: completo
					if($module->id_client_submodule == 23 || $module->id_client_submodule == 25 || $module->id_client_submodule == 26 || $module->id_client_submodule == 27){
						$disabled_add_delete = "disabled";
					} else {
						$disabled_add_delete = "";
					}
                ...
                <?php } ?>
					...
					<?php if($module->id_client_module == 15) { // ADMINISTRACIÓN CLIENTE - CARGA MASIVA MIMAire ?>
						<?php if($module->id_client_submodule == 27){ continue; } // SUBMÓDULOS CARGA MASIVA MIMASOFT ?>
					<?php } ?>
