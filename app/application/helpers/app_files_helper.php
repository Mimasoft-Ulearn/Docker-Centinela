...
if (!function_exists('upload_file_to_temp')) {
    ...
    function upload_file_to_temp($name = 'file', $options = array()) {
        
        if (!empty($_FILES)) {
            $temp_file = $_FILES[$name]['tmp_name'];
			//$file_name = $_FILES[$name]['name'];
            //$file_name = uniqid("file")."-".$_FILES[$name]['name'];
			
			$retirement_file = get_array_value($options, "retirement_file");
            $reception_file = get_array_value($options, "reception_file");
            $id_campo = get_array_value($options, "id_campo");
			
			if($retirement_file) {
				$file_name = "retirement_file-".$_FILES[$name]['name'];
			} elseif($reception_file) {
				$file_name = "reception_file-".$_FILES[$name]['name'];
			} else {
				$file_name = $_FILES[$name]['name'];
            }
            ...
            $target_file = $target_path . $prefix.$file_name;
...
if (!function_exists('download_app_files')) {
    ...
    function download_app_files($directory_path, $serialized_file_data, $remove_prefix = true, $final_name = NULL) {
        $ci = get_instance();
        ...
        if ($file_exists) {
            $final_name = (!$final_name) ? lang('download_zip_name') : $final_name;
            $ci->zip->download($final_name . '.zip');
        } else {
        ...
        if ($serialized_file_data && $file_path) {
            ...
            foreach ($files_array as $file) {
                ...
                if (in_array($file_name, $delete_files)) {
                    delete_file_from_directory($file_path . $file_name);
                } else {
