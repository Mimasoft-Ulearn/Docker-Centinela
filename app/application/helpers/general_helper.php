...
if (!function_exists('active_menu')) {
    ...
    function active_menu($menu = "", $submenu = array(), $param = "") {
        $ci = & get_instance();
        ...
        if ($menu === $controller_name) {
            ...
            foreach ($submenu as $sub_menu) {
                if (strtolower(get_array_value($sub_menu, "controller")) === $controller_name) {
                    return "active";
                } else if (get_array_value($sub_menu, "category") === $controller_name) {
                    return "active";
                }
            }
        } else if($controller_name == "air_forecast_sectors" && $param_decrypt == $uri_decrypt) { // Excepción para módulo de Pronósticos y módulos dinámicos de sectores

            $uri = $ci->uri->segment(3);

            $param_decrypt = rawurldecode($ci->encrypt->decode(rawurldecode($param)));
            $uri_decrypt = rawurldecode($ci->encrypt->decode(rawurldecode($uri)));
            
            if($param_decrypt == $uri_decrypt){
                return "active";
            }
...
if (!function_exists('active_submenu')) {
    ...
    function active_submenu($submenu = "", $is_controller = false, $param = "") {

        $ci = & get_instance();

        //if submenu is a controller then compare with controller name, otherwise compare with method name
        if ($is_controller && strtolower($submenu) === strtolower(get_class($ci))) {

            return "active";
        
        } else if($submenu == "air_forecast_sectors" && $param_decrypt == $uri_decrypt) { // Excepción para módulo de Pronósticos y submódulos dinámicos de sectores
 ...
 * @return config value
 */
if (!function_exists('get_setting_mimasoft')) {

    function get_setting_mimasoft($id_proyecto, $configuracion) {
        $ci = & get_instance();
	
		$settings = $ci->General_settings_model->get_one_where(array("id_proyecto" => $id_proyecto, "deleted" => 0));
		//$configuracion = "date_format";
...
if (!function_exists('send_app_mail_PHPMailer')) {

    function send_app_mail_PHPMailer($to, $subject, $message, $optoins = array()) {
        ...
        if (is_array($attachments)) {
            foreach ($attachments as $value) {
                ...
                $mail->addAttachment(trim($file_path), $file_name);
            }
        }
...
/**
 * Esta funcion permite sincronizar la tabla client_waste_settings con las categorias de residuosdeun proyecto especifico
 * 
 * @return boolean
 */
 /*
if (!function_exists('synchronize_waste_settings')) {

    function synchronize_waste_settings($id_cliente = NULL, $id_proyecto = NULL) {
        $ci = get_instance();
		
		if($id_cliente && $id_proyecto){
			
			// CONSULTO TODAS LAS CATEGORIAS SIENDO USADAS EN LOS FORMULARIOS DEL PROYECTO
			$categorias_residuo = $ci->Form_rel_materiales_rel_categorias_model->get_categories_of_form($id_proyecto, "Residuo")->result();
			foreach($categorias_residuo as $index => $form_mat_cat){
				$id_formulario = $form_mat_cat->id_formulario;
				$id_material = $form_mat_cat->id_material;
				$id_categoria = $form_mat_cat->id_categoria;
				
				// POR CADA CATEGORIA BUSCO SUS FILAS CON ESTADO NO BORRADAS EN EL SETTING DE RESIDUOS
				$busqueda = $ci->Client_waste_settings_model->get_all_where(
					array(
				...
				// SI ENCUENTRA 1 O MÁS FILAS ASOCIADAS A ESA CATEGORIA
				if($busqueda){
					
					// LAS RECORRO PARA BORRAR LAS FILAS REPETIDAS CON LOS MISMOS VALORES
					foreach($busqueda as $index => $client_waste_settings){
						if($index > 0){
							$id_setting = $client_waste_settings->id;
							$ci->Client_waste_settings_model->real_delete($id_setting);
						}
					}
					
				}else{// SI NO ENCUENTRA FILAS ASOCIADAS A ESA CATEGORIA
					
					// BUSCO SUS FILAS CON ESTADO BORRADAS EN EL SETTING DE RESIDUOS
					$busqueda = $ci->Client_waste_settings_model->get_one_where(
						array(
							"id_proyecto" => $id_proyecto,
							"id_categoria" => $id_categoria,
							"deleted" => 1
					...
					// SI NO ENCUENTRA RESULTADOS, AGREGO LA FILA A LA TABLA
					if(!$busqueda->id){
						
						$options = array("id_cliente" => $id_cliente, "id_proyecto" => $id_proyecto, "id_categoria" => $id_categoria, "tabla" => 0,"grafico" => 0, "deleted" => 0);
						$save_id = $ci->Client_waste_settings_model->save($options);
 ...
 */
 /*
if (!function_exists('client_consumptions_settings')) {

    function client_consumptions_settings($id_cliente = NULL, $id_proyecto = NULL) {
        $ci = get_instance();
		
		if($id_cliente && $id_proyecto){
			
			// CONSULTO TODAS LAS CATEGORIAS SIENDO USADAS EN LOS FORMULARIOS DEL PROYECTO
			$categorias_consumo = $ci->Form_rel_materiales_rel_categorias_model->get_categories_of_form($id_proyecto, "Consumo")->result();
			foreach($categorias_consumo as $index => $form_mat_cat){
				$id_formulario = $form_mat_cat->id_formulario;
				$id_material = $form_mat_cat->id_material;
				$id_categoria = $form_mat_cat->id_categoria;
				
				// POR CADA CATEGORIA BUSCO SUS FILAS CON ESTADO NO BORRADAS EN EL SETTING DE RESIDUOS
				$busqueda = $ci->Client_consumptions_settings_model->get_all_where(
					array(
						"id_proyecto" => $id_proyecto,
						"id_categoria" => $id_categoria,
						"deleted" => 0
				...
				if($busqueda){
					...
					foreach($busqueda as $index => $client_waste_settings){
						if($index > 0){
							$id_setting = $client_waste_settings->id;
							$ci->Client_consumptions_settings_model->real_delete($id_setting);
						}
					}
					...
					// BUSCO SUS FILAS CON ESTADO BORRADAS EN EL SETTING DE RESIDUOS
					$busqueda = $ci->Client_consumptions_settings_model->get_one_where(
						array(
							"id_proyecto" => $id_proyecto,
							"id_categoria" => $id_categoria,
							"deleted" => 1
					...
					if(!$busqueda->id){
						...
						$options = array("id_cliente" => $id_cliente, "id_proyecto" => $id_proyecto, "id_categoria" => $id_categoria, "tabla" => 0,"grafico" => 0, "deleted" => 0);
						$save_id = $ci->Client_consumptions_settings_model->save($options);
...
/**
 * clean a text removing special chars and replacing spaces with underscore
 * 
 * @param String $string
 * @return String
 */
if (!function_exists('clean')) {
    ...
    function clean($string, $lower = TRUE){
 ...
 * @return config value
 */
if (!function_exists('get_setting_client_mimasoft')) {

    function get_setting_client_mimasoft($id_cliente, $configuracion) {
        $ci = & get_instance();
	
		$settings = $ci->General_settings_clients_model->get_one_where(array("id_cliente" => $id_cliente, "deleted" => 0));
		//$configuracion = "date_format";
		return $settings->$configuracion;
		//return $settings["date_format"];
    }
